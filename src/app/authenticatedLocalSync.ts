...
export function syncAuthenticatedLocalState(
  ...
  if (dbPreference) {
    ...
    // source for dailyReminder/startTime (ADR-004). It is not
    // preserved from whatever the local screening cache happened to hold,
    // because that would create two independent sources. No row yet means
    // the product default (not enabled), never an inferred value from an
    // old anonymous cache.
    saveScreening({
      ...dbPreference,
